...
<div class="row">
      ...
      $sql= SELECT `customers`.`firstName` AS `FirstName`, `customers`.`lastName` AS `LastName`, `jobs`.`customerID` AS `CustomerID`
FROM `customers` ORDER BY `customers`.`lastName` ASC;
      ...
      if (mysqli_num_rows($result) > 0) {
          ...
          while($row = mysqli_fetch_assoc($result)) {
              ...
              echo "custmerID: " . $row["customerID"]. "<br><hr>";
        }
      } else {
        echo "0 results";
      }
